...
{
    public class GetWarzoneServiceRecordsTests : BaseHaloAPIServiceTests
    {
        [Test]
        public void Default_DoesNotThrowException()
        {
            Assert.DoesNotThrow(async () => await HaloApiService.GetWarzoneServiceRecords(new[] {"Glitch100"}));
        }

        [Test]
        public async void Default_DoesNotReturnNull()
        {
            var result = await HaloApiService.GetWarzoneServiceRecords(new[] { "Glitch100" });
        ...
        {
            var result = await HaloApiService.GetWarzoneServiceRecords(new[] { "ASFASFEAFEA" });
