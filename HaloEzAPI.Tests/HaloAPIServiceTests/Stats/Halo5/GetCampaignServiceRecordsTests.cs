...
{
    public class GetCampaignServiceRecordsTests : BaseHaloAPIServiceTests
    {
        [Test]
        public void Default_DoesNotThrowException()
        {
            Assert.DoesNotThrow(async () => await HaloApiService.GetCampaignServiceRecords(new[] { "Glitch100" }));
        }
        ...
        {
            var result = await HaloApiService.GetCampaignServiceRecords(new[] { "Glitch100" });
        ...
        {
            var result = await HaloApiService.GetCampaignServiceRecords(new[] { "ASFASFEAFEA" });
        ...
        {
            var result = await HaloApiService.GetCampaignServiceRecords(new[] { gamertag1, gamertag2, gamertag3 });
